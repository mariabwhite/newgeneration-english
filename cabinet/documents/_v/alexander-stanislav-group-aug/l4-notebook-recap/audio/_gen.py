"""
L4 · Massive Trainer · edge-tts mp3 generator
Voice: en-GB-SoniaNeural (British female).
Scans index.html for all `data-say="..."` attributes AND all `w:` / `right:` / `examples:` strings
inside the JS data blocks. Generates one mp3 per unique text and writes _manifest.js.
"""
...
def slug(text: str) -> str:
    """Short filesystem-safe file name."""
    base = re.sub(r"[^\w\s-]", "", text.lower()).strip()
    base = re.sub(r"[\s_-]+", "-", base)
    if len(base) > 60:
        # keep first 50 chars + hash suffix for uniqueness
        h = hashlib.md5(text.encode("utf-8")).hexdigest()[:8]
        base = base[:50].rstrip("-") + "-" + h
    return base + ".mp3"
