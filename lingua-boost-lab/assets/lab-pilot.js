...
(function(){
  ...
  ready(function(){
    // v5 · 2026-07-15 · sync селектора с lab-homework.js v48 — раньше лифт не строился
    // на 14 Grammar Arcade (`arcade-mission`+`canon-l-why`+`rules-table`+`time-sort-*`),
    // 09 English Booster (`booster-part`+`booster-map`+`booster-report`),
    // 11 Ancient China (`card canon-l-card`), 16 Whispering (`wl-chapter`+`wl-comic`),
    // 12 Menu Lab (`mission-card`), потому что нужно ≥ 2 секций → лифт нет.
    // Теперь тот же полный список что и у homework-декоратора.
    const sections = [...document.querySelectorAll('section.section, section.lab-section, section.lesson-section, section.block, section.canon-l-section, section.canon-l-card, section.card, section[class*="section-"], section[id^="block-"], section[id^="block"], section[id^="sec-"], section[id^="sec"], section[id^="section-"], section[id^="b"][id$="1"], section[id^="b"][id$="2"], section[id^="b"][id$="3"], section[id^="b"][id$="4"], section[id^="b"][id$="5"], section[id^="b"][id$="6"], section[id^="b"][id$="7"], section[id^="b"][id$="8"], section[id^="b"][id$="9"], section[class*="mission"], section.grammar-map, section.wl-chapter, section.wl-finale, section.wl-comic, section.intro-reading, section[class*="booster-part"], section.booster-map, section.booster-report, section.canon-l-why, section.rules-table, section.vocab-panel, section.now-result-zone, section.time-sort-bank, section.time-sort-box, section.passport-finale, .lesson-section, .lab-block, .lb-block')];
